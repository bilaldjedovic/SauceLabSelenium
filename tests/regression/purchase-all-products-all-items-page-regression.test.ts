...
describe("User can checkout with all items - regression", () => {
  ...
  test.each(users)(
    'Checkout with all products from "All items" page',
    async (user) => {
      ...
      await checkoutOverviewPage.validateCheckoutOverviewPageTitle(
        "Checkout: Overview"
      );
      await checkoutOverviewPage.clickFinishButton();

      await checkoutCompletePage.checkIsAtCheckoutComplete();
      await checkoutCompletePage.validateCheckoutCompleted(
